namespace new_wr_api.Dto
{
    public class DuLieuNguonNuocThaiSinhHoatDto
    {
        ...
        public double? LtSinhHoatBOD => Math.Round((SoDan ?? 0) * (CtSinhHoatBOD ?? 0) * (HeSoSuyGiam ?? 0));
        public double? LtSinhHoatCOD => Math.Round((SoDan ?? 0) * (CtSinhHoatCOD ?? 0) * (HeSoSuyGiam ?? 0));
        public double? LtSinhHoatAmoni => Math.Round((SoDan ?? 0) * (CtSinhHoatAmoni ?? 0) * (HeSoSuyGiam ?? 0));
        public double? LtSinhHoatTongN => Math.Round((SoDan ?? 0) * (CtSinhHoatTongN ?? 0) * (HeSoSuyGiam ?? 0));
        public double? LtSinhHoatTongP => Math.Round((SoDan ?? 0) * (CtSinhHoatTongP ?? 0) * (HeSoSuyGiam ?? 0));
        public double? LtSinhHoatTSS => Math.Round((SoDan ?? 0) * (CtSinhHoatTSS ?? 0) * (HeSoSuyGiam ?? 0));
        public double? LtSinhHoatColiform => Math.Round((SoDan ?? 0) * (CtSinhHoatColiform ?? 0) * (HeSoSuyGiam ?? 0));
